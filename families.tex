...
{
\draw[green!50!gray] (c1) -- (c3);
\draw[green!50!gray] (c3) -- (c2);
}
...
\firstPicture
\findFirstIntersection
\draw[gray,opacity=.5] (p1) -- (c1);
\draw[gray,opacity=.5] (p3) -- (c1);
\fill[brown!50] (c1) circle (1.5pt);
}
...
\firstPicture
\findSecondIntersection
\draw[gray,opacity=.5] (p1) -- (c2);
\draw[gray,opacity=.5] (p5) -- (c2);
\fill[brown!50] (c2) circle (1.5pt);
}
...
\firstPicture
\findThirdIntersection
\draw[gray,opacity=.5] (p2) -- (c3);
\draw[gray,opacity=.5] (p0) -- (c3);
\fill[brown!50] (c3) circle (1.5pt);
}
...
Since each side has a ``successor'', and there is an even number of sides, each side has an ``opposite'' side, half way around the hexagon.
...
\begin{example}
Through any 2 points there is precisely one line: \begin{tikzpicture}[scale=.5] \draw[gray!50] (0,0) -- (1,0); \fill[gray!50] (0,0) circle (1.3pt); \fill[gray!50] (1,0) circle (1.3pt); \end{tikzpicture}
\end{example}
\begin{example}
