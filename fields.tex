...
\end{problem}
An \emph{isomorphism}\define{isomorphism} of rings is a bijection \(\varphi\colon S\to T\) of rings matching up their addition and multiplication operations.
Rings are \emph{isomorphic}\define{isomorphic} if there is an isomorphism between them.
...
\begin{example}
If \(S\) is a commutative ring with no zero divisors, a \emph{rational function}\define{rational!function}\define{function!rational} with coefficients drawn from \(S\) is a formal ratio
\[
\frac{b(x)}{c(x)}
\]
...
A \emph{zero divisor}\define{zero divisor} in a ring \(R\) is an element \(r\ne 0\) so that \(rs=0\) for some element \(s\ne 0\) of \(R\).
