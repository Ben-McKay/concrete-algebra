...
If there are only finitely many nonzero terms, the map is a \emph{polynomial map}\define{polynomial!map}\define{map!polynomial} or \emph{regular map}.
...
\]
\end{proof}
\begin{theorem}[Euclidean algorithm]\label{thm:EA.fps}
...
\end{proof}
A \emph{Weierstrass polynomial}\define{Weierstrass polynomial} in variables \(x=(x_1,\dots,x_n)\) and a variable \(y\) is a formal power series 
\[
...
\begin{theorem}[Weierstrass preparation]\define{Weierstrass preparation theorem}\define{theorem!Weierstrass preparation}
Over a field, take a formal power series \(f(x,y)\) in \(n\) variables \(x=(x_1,\dots,x_n)\) and one variable \(y\)
\[
f(x,y)=\sum a_j(x)y^j.
\]
