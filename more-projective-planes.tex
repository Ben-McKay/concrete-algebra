...
\newcommand*{\AthreeX}{1.5}
\newcommand*{\AthreeY}{4.5}
\newcommand*{\Athree}{\AthreeX,\AthreeY}
\newcommand*{\PnoughtX}{0}
\newcommand*{\PnoughtY}{8}
...
{
\fill[brown!15,opacity=.5] (\Bone) -- (\Btwo) -- (\Bthree) -- cycle;
\fill[brown!50,opacity=.5] (\Bone) circle (\dotr) node (b1) {};
\fill[brown!50,opacity=.5] (\Btwo) circle (\dotr) node (b2) {};
\fill[brown!50,opacity=.5] (\Bthree) circle (\dotr) node (b3) {};
}
...
{
\draw[red!50!white] (a3) -- (c1); % line through c1
\draw[red!50!white] (b3) -- (c1); % line through c1
\showFirstIntersectionPoint
}

\newcommand*{\showHowSecondIntersectionIsConstructed}
{
\draw[blue] (a3) -- (c2); % line through c1
\draw[blue] (b3) -- (c2); % line through c1
\showSecondIntersectionPoint
}

\newcommand*{\showHowThirdIntersectionIsConstructed}
{
\draw[brown] (a2) -- (c3); % line through c1
\draw[brown] (b2) -- (c3); % line through c1
\showThirdIntersectionPoint
}
...
%Take a quadrilateral of points, but picture them as consisting of a triangle, and a point not lying on any side of that triangle, which we call the origin.
%\Desargue{1}
%For each of the 3 vertices, draw the lines through the origin and that vertex.
%Pick any 3 points on those lines, forming some triangle.
...
\Desargue{2}
The two triangles are \emph{in perspective}\define{perspective} from a line \(\ell\) if corresponding edges intersect on \(\ell\).
\Desargue{9}
%See how the vertices of the original triangle correspond, via these lines, with those of the new triangle.
%We say that the two triangles are \emph{in perspective} from the origin point.
%Given two triangles, with a correspondence between their vertices, we can always find the origin, if it exists, by intersecting the lines through corresponding vertices.
%Declare sides to correspond if they pass through corresponding vertices.
%(We could easily generalise this story to consider complicated drawings being ``in perspective'' from a point.)

%Take intersections of each pair of corresponding sides: draw the intersection of the first pair
%\Desargue{3}
%and of the second pair
%\Desargue{4}
%and of the third pair, giving us three intersection points.
%\Desargue{5}
...
\newcommand{\lne}[4]{\draw[gray!50] (#1,#2) -- (#3,#4);}
\newenvironment{ppdiagram}%
{%%
\begin{center}
\begin{tikzpicture}[scale=.4]
%\draw[white] (-0.5,-0.5) grid (3.5,3.5); 
