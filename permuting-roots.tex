...
A polynomial \(f(t_1,t_2,\dots,t_n)\) is \emph{symmetric}\define{symmetric!polynomial}\define{polynomial!symmetri c} if it is unchanged by permuting the variables \(t_1, t_2, \dots, t_n\).
\begin{example}
...
Order terms by ``alphabetical'' order, also called \emph{weight}\define{weight}: order monomials by how much \(t_1\); if two monomials have the same number of \(t_1\) factors, break the tie by looking at \(t_2\) factors, and so on.
\begin{example}
The term
\[
t_1^5 t_2^3 
...
Over any field, a rational function \(f(A)\) in the entries of a square matrix \(A\), with coefficients in the field, is \emph{invariant}\define{invariant!of square matrix} if \(f(FAF^{-1})=f(A)\) for any invertible matrix \(F\) with coefficients in the field.
