...
1, x+1, 5+3x^2-2x,
\]
a finite sum of terms, each the product of a number (the \emph{coefficient}\define{coefficient!of polynomial} of the term), and a nonnegative integer power of \(x\) (the \emph{degree} of the term).
We define the degree of a constant to be \(0\), except that the degree of \(0\) is defined to be \(-\infty\).
We can add together coefficients of terms of the same degree:
\[
2x^2+7x^2+x=(2+7)x^2+x=9x^2+x.
\]
We usually arrange that the terms decrease in degree, writing \(1+5x+x^2\) as \(x^2+5x+1\).
When we write a polynomial this way, the \emph{degree}\define{degree!of polynomial} is the largest degree of any term.
We might ask the coefficients to be integers, rational numbers, real numbers, or complex numbers (in which case, we usually use \(z\) as the name of the variable, instead of \(x\)).
Bizarrely, we might even allow the coefficients to be remainders modulo some integer.
As soon as we fix on any of these choices of coefficients, the addition laws, multiplication laws, and distributive law (familiar from chapter~\ref{chapter:integers}) are immediately obvious, just writing \(b(x)\) as a polynomial, instead of an integer \(b\), and so on, and changing the word \emph{integer} to \emph{polynomial}.
We will ignore the sign laws and the law of well ordering.
...
\emph{Danger:} if we ask that \(x\) also be a remainder modulo \(2\), we might want to think of this polynomial as a \emph{function} of a variable \(x\).
...
\]
modulo \(2\).
So a polynomial with coefficients in remainders modulo an integer is \emph{not} just a function.
It is a purely algebraic object.
\end{example}
...
A \emph{monic polynomial}\define{monic polynomial}\define{polynomial!monic} is one whose highest order term has coefficient \(1\): the greatest common divisor will always be taken to be a monic polynomial, and then there is a unique greatest common divisor.
\end{example}
