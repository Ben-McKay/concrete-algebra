...
\]
has a real root \(x\).
More generally, prove that any odd-order polynomial equation in a real variable has a real root.
Try letting \(u=-x\); plug in to see if has any negative real root.
\end{problem}
\section{Counting positive roots}
\epigraph{A biologist, a physicist and a mathematician drink coffee and watch a house across the street from them. They see two people walk into the house.
An hour later, three people walk out.
\\
    \emph{Physicist:} We measured incorrectly.
\\
    \emph{Biologist:} No, they reproduced.
\\
    \emph{Mathematician:} No, if one more person enters, the house will be empty.}
...
So as we travel along the signs, they change twice:
\[
\input{sign-changes}
\]
once from \(+\) to neighboring \(-\), and once from \(-\) to neighboring \(+\).
...
\end{problem*}
\begin{answer}{real.polynomials:unique.p.f.d}
