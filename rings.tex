...
\section{Definition}
A \emph{morphism}\define{morphism!ring} of rings \(\map{f}{R}{S}\) is a map \(f\) taking elements of a ring \(R\) to elements of a ring \(S\), so that \(f(a+b)=f(a)+f(b)\) and \(f(ab)=f(a)f(b)\) for all elements \(b, c\) of \(R\).
(Morphisms are also often called \emph{homomorphisms}.\define{homomorphism})
\begin{example}
The map \(\map{f}{\Z{}}{\Zmod{m}}\), \(f(b)=\bar{b}\), remainder modulo \(m\), is a morphism.
\end{example}
\begin{problem}{rings:check.morphism}
Prove that the map \(\map{f}{\Zmod{3}}{\Zmod{6}}\), \(f(b)=4b\) is a morphism.
Draw pictures to explain it.
\end{problem}
\begin{problem}{rings:find.morphisms}
Find all morphisms \(\map{f}{\Zmod{p}}{\Zmod{q}}\), where \(p\) and \(q\) are prime numbers and \(p \ne q\).
\end{problem}
\section{Sage}
We define \(R=\Q{}[x,y]\) and \(T=\Q{}[z]\) and define a morphism \(\map{\phi}{R}{S}\) by \(\phi(x)=z^3\) and \(\phi(y)=0\):
\begin{sageblock}
...
If \(\map{f}{R}{S}\) is a morphism of rings, the \emph{kernel}\define{kernel!ring morphism} of \(f\) is the set 
\[
...
Prove that \(f=g\).
\end{problem}
A morphism \(\map{f}{R}{S}\) with kernel \(0\) is \emph{injective}\define{injective!morphism}\define{morphism!injective}, also called \emph{one-to-one}\define{one-to-one!morphism}\define{morphism!one-to-one}, while a morphism \(\map{f}{R}{S}\) with image \(f(R)=S\) is \emph{surjective}\define{surjective!morphism}\define{morphism!surjective} or \emph{onto}\define{morphism!onto}\define{onto!morphism}.
...
An isomorphism \(\map{f}{R}{R}\) from a ring to itself is an \emph{automorphism}.\define{automorphism!ring}
\begin{example}
